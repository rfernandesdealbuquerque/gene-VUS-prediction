...
def get_best_hyperparameters(results_df) -> pd.DataFrame:
    ...
    median_AUC_overall = results_df['Test AUC'].median()
    ...
    best_hyperparameters.rename(columns={'mean': 'best params avg AUC'}, inplace=True)
    best_hyperparameters['overall avg AUC'] = round(mean_AUC_overall, 2)
    best_hyperparameters['overall median AUC'] = round(median_AUC_overall, 2)
