...
def get_median_model_seed(best_hyperparameters, results_df):
    '''Returns seed of the median model among the models with the best hyperparameters group.'''
    filtered_df = results_df

    print(list(best_hyperparameters.columns)[:-3])
    for col in list(best_hyperparameters.columns)[:-3]:
        filtered_df = filtered_df[filtered_df[f'{col}'] == best_hyperparameters.at[0, f'{col}']]
    print(filtered_df)
